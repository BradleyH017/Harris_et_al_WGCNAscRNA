# Bradley August 2021
# scRNA-Seq - tidied pipeline, kArray to explore the optimum number of nearest neighbours to use
...
do_clustering = T
generate_markers = T
enrichment_vs_authors = F


# Do clustering if wanted
if(do_clustering == T){
        # load data
        load(paste(pathOut, "objects/seur_integrated_UMAPd.Rds", sep = "/"))
        # Do clustering
...
#       save(seur.integrated, file = paste(pathOut, "/kArray/seur.integrated.clusterd.k", k.param, ".Rds", sep = ""))
}
...
if(generate_markers == T){
        ...
        # Save
        write.csv(markers, paste(pathOut, "/tables/markers/", k.param, ".csv", sep = ""))
} else {
        markers <- read.csv(paste(pathOut, "/tables/markers/", k.param, ".csv", sep = ""))
}
...
if(enrichment_vs_authors == T){
        ...
        gene_list_dir <- paste(pathOut, "tables/markers", sep ="/")
        ...
        write.csv(fgsea_Res, paste(pathOut, "/tables/markers/", "k", k.param, ".fgsea_vs_authors.csv", sep = ""))
}
